...
__global__ void clear_scalar_kernel(CellCloudView cloud, float* field) {
  for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < static_cast<int>(cloud.size_y); y += blockDim.y * gridDim.y) {
    for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < static_cast<int>(cloud.size_x); x += blockDim.x * gridDim.x) {
      field[index_2d(x, y, static_cast<int>(cloud.size_x))] = 0.0f;
    }
  }
}

__global__ void add_source_kernel(CellCloudView cloud, float dt, float time, float dx, float dy,
                                  float reference_density) {
  const float domain_width = static_cast<float>(cloud.size_x) * dx;
  const float domain_height = static_cast<float>(cloud.size_y) * dy;
  ...
  for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < static_cast<int>(cloud.size_y); y += blockDim.y * gridDim.y) {
    for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < static_cast<int>(cloud.size_x); x += blockDim.x * gridDim.x) {
      const float cell_x = (static_cast<float>(x) + 0.5f) * dx;
      const float cell_y = (static_cast<float>(y) + 0.5f) * dy;
...
__global__ void diffuse_state_kernel(CellCloudView cloud, float dt, float dx, float dy,
                                     float kinematic_viscosity, float density_diffusivity,
                                     float reference_density) {
  ...
  for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < static_cast<int>(cloud.size_y); y += blockDim.y * gridDim.y) {
    for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < static_cast<int>(cloud.size_x); x += blockDim.x * gridDim.x) {
      ...
      const float lap_density = (left.density_offset - 2.0f * center.density_offset + right.density_offset) * inv_dx2 +
                                (down.density_offset - 2.0f * center.density_offset + up.density_offset) * inv_dy2;
      const float lap_vel_x = (left.velocity.x - 2.0f * center.velocity.x + right.velocity.x) * inv_dx2 +
                              (down.velocity.x - 2.0f * center.velocity.x + up.velocity.x) * inv_dy2;
      const float lap_vel_y = (left.velocity.y - 2.0f * center.velocity.y + right.velocity.y) * inv_dx2 +
                              (down.velocity.y - 2.0f * center.velocity.y + up.velocity.y) * inv_dy2;

      cloud.cell_state_tmp[idx].density_offset = clamp_density_offset(
          center.density_offset + density_diffusivity * dt * lap_density, reference_density);
...
__global__ void advect_state_kernel(CellCloudView cloud, float dt, float dx, float dy,
                                    float reference_density) {
  for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < static_cast<int>(cloud.size_y); y += blockDim.y * gridDim.y) {
    for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < static_cast<int>(cloud.size_x); x += blockDim.x * gridDim.x) {
      const int idx = index_2d(x, y, static_cast<int>(cloud.size_x));
      const float backtrace_x = static_cast<float>(x) - dt * cloud.cell_state[idx].velocity.x / dx;
      const float backtrace_y = static_cast<float>(y) - dt * cloud.cell_state[idx].velocity.y / dy;
      cloud.cell_state_tmp[idx].density_offset = clamp_density_offset(
          sample_density_offset(cloud.cell_state, backtrace_x, backtrace_y, static_cast<int>(cloud.size_x),
                                static_cast<int>(cloud.size_y)),
          reference_density);
      cloud.cell_state_tmp[idx].velocity = sample_velocity(cloud.cell_state, backtrace_x, backtrace_y,
                                                           static_cast<int>(cloud.size_x), static_cast<int>(cloud.size_y));
      cloud.cell_state_tmp[idx].velocity.z = 0.0f;
    }
  }
}

__global__ void compute_divergence_kernel(CellCloudView cloud, float dx, float dy) {
  ...
  for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < static_cast<int>(cloud.size_y); y += blockDim.y * gridDim.y) {
    for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < static_cast<int>(cloud.size_x); x += blockDim.x * gridDim.x) {
      ...
      const float dudx = (cloud.cell_state[index_2d(x + 1, y, static_cast<int>(cloud.size_x))].velocity.x -
                          cloud.cell_state[index_2d(x - 1, y, static_cast<int>(cloud.size_x))].velocity.x) * inv_2dx;
      const float dvdy = (cloud.cell_state[index_2d(x, y + 1, static_cast<int>(cloud.size_x))].velocity.y -
                          cloud.cell_state[index_2d(x, y - 1, static_cast<int>(cloud.size_x))].velocity.y) * inv_2dy;
      cloud.divergence[idx] = dudx + dvdy;
    }
  }
}

__global__ void pressure_jacobi_kernel(CellCloudView cloud, float dx, float dy) {
  const float dx2 = dx * dx;
  const float dy2 = dy * dy;
  const float denominator = 2.0f * (dx2 + dy2);
  ...
  for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < static_cast<int>(cloud.size_y); y += blockDim.y * gridDim.y) {
    for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < static_cast<int>(cloud.size_x); x += blockDim.x * gridDim.x) {
      ...
      const float px = (cloud.pressure[index_2d(x - 1, y, static_cast<int>(cloud.size_x))] +
                        cloud.pressure[index_2d(x + 1, y, static_cast<int>(cloud.size_x))]) * dy2;
      const float py = (cloud.pressure[index_2d(x, y - 1, static_cast<int>(cloud.size_x))] +
                        cloud.pressure[index_2d(x, y + 1, static_cast<int>(cloud.size_x))]) * dx2;
      cloud.pressure_tmp[idx] = (px + py - cloud.divergence[idx] * dx2 * dy2) / denominator;
    }
  }
}

__global__ void project_velocity_kernel(CellCloudView cloud, float dx, float dy) {
  const float inv_2dx = 0.5f / dx;
  const float inv_2dy = 0.5f / dy;
  ...
  for (int y = blockIdx.y * blockDim.y + threadIdx.y; y < static_cast<int>(cloud.size_y); y += blockDim.y * gridDim.y) {
    for (int x = blockIdx.x * blockDim.x + threadIdx.x; x < static_cast<int>(cloud.size_x); x += blockDim.x * gridDim.x) {
      ...
      const float grad_x = (cloud.pressure[index_2d(x + 1, y, static_cast<int>(cloud.size_x))] -
                            cloud.pressure[index_2d(x - 1, y, static_cast<int>(cloud.size_x))]) * inv_2dx;
      const float grad_y = (cloud.pressure[index_2d(x, y + 1, static_cast<int>(cloud.size_x))] -
                            cloud.pressure[index_2d(x, y - 1, static_cast<int>(cloud.size_x))]) * inv_2dy;
...
double Simulation::compute_time_step() const {
  ...
  const float max_speed = thrust::transform_reduce(thrust::device, begin, end, MaxAbsVelocity{}, 0.0f,
                                                   thrust::maximum<float>{});

  const double characteristic_speed = std::max(static_cast<double>(max_speed), 1.0);
  const double cell_length = std::min(config_.dx, config_.dy);
  return config_.cfl * cell_length / characteristic_speed;
}
...
void Simulation::step(double max_dt) {
  ...
  const float dx = static_cast<float>(config_.dx);
  const float dy = static_cast<float>(config_.dy);
  ...
  add_source_kernel<<<launch.grid, launch.block>>>(cloud, dt, current_time, dx, dy, reference_density);
  CUDA_CHECK(cudaGetLastError());

  diffuse_state_kernel<<<launch.grid, launch.block>>>(cloud, dt, dx, dy, kinematic_viscosity,
                                                      density_diffusivity, reference_density);
  CUDA_CHECK(cudaGetLastError());
  std::swap(cloud_.cell_state, cloud_.cell_state_tmp);
  cloud = cloud_.view();

  advect_state_kernel<<<launch.grid, launch.block>>>(cloud, dt, dx, dy, reference_density);
  CUDA_CHECK(cudaGetLastError());
  std::swap(cloud_.cell_state, cloud_.cell_state_tmp);
  cloud = cloud_.view();
  ...
  compute_divergence_kernel<<<launch.grid, launch.block>>>(cloud, dx, dy);
  CUDA_CHECK(cudaGetLastError());
  ...
  for (int iter = 0; iter < config_.pressure_iterations; ++iter) {
    pressure_jacobi_kernel<<<launch.grid, launch.block>>>(cloud, dx, dy);
    CUDA_CHECK(cudaGetLastError());
    std::swap(cloud_.pressure, cloud_.pressure_tmp);
    cloud = cloud_.view();
  }

  project_velocity_kernel<<<launch.grid, launch.block>>>(cloud, dx, dy);
  CUDA_CHECK(cudaGetLastError());
...
HostState Simulation::download_state() const {
  ...
  host_state.velocity_x.resize(device_state.size());
  host_state.velocity_y.resize(device_state.size());

  for (std::size_t i = 0; i < device_state.size(); ++i) {
    host_state.density_offset[i] = device_state[i].density_offset;
    host_state.velocity_x[i] = device_state[i].velocity.x;
    host_state.velocity_y[i] = device_state[i].velocity.y;
  }
